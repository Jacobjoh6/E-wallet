...
function Card() {
    ...
    const [cardnr, setCardnr] = useState(navigationState.state.card.cardnr)
    const [name, setName]     = useState(navigationState.state.card.name)
    const [date, setDate]     = useState(navigationState.state.card.date)
    const [ccv, setCcv]       = useState(navigationState.state.card.ccv)
    ...
    return(
       ...
       { navigationState?.state?.card? 
       <article>
        <h1 className='home-view__card__numbers' defaultValue={ cardnr } onChange={ (event) => setCardnr(event.target.value) }>1234  5678  9101  1121</h1>
        ...
        <article className='home-view__card__info'>
            <h2 className='home-view__card__label__h2' defaultValue={ name } onChange={ (event) => setName(event.target.value) }>JACOB JOHANSSON</h2>
            <h2 className='home-view__card__label__h2' defaultValue={ date } onChange={ (event) => setDate(event.target.value) }>10/10</h2>
        </article>
        </article>
        : <p>nåt gick felt</p>
       }     
